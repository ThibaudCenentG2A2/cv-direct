...
class CV_PDF
{
    ...
    {
        $req = BD::getInstance()->prepare('SELECT * FROM CV_PDF WHERE ID_CV_PDF = :id_cv_pdf AND ID_CV = :id_cv');
    ...
    {
        $req = BD::getInstance()->prepare('SELECT COUNT(ID_CV_PDF) AS nbre_CV_PDF FROM CV_PDF WHERE ID_CV = :id_cv');
        $req->execute(array('id_cv' => $this->get_id_cv()));
        $donnees = $req->fetch();
