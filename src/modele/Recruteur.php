...
class Recruteur
{
    ...
    public static function inscrire_recruteur($pseudo, $prenom, $nom, $mail, $mdp)
    {
        ...
        {
            $req = BD::getInstance()->prepare('INSERT INTO RECRUTEUR(PSEUDONYME, PRENOM, NOM, EMAIL, PASSWORD, VALID) VALUES (:pseudo, :prenom, :nom, :mail, :mdp, 0)');// a modifier selon la bd hein :)
    ...
    {
        $req = BD::getInstance()->prepare('SELECT COUNT(*) AS TOTAL FROM RECRUTEUR WHERE EMAIL = :mail');
        $req->execute(array("mail" => $mail));
        $res = $req->fetch();
    ...
    static function modifier_mot_de_passe($mail, $nouveau_mdp)
    {
        ...
        else
        {
            ...
            $req = BD::getInstance()->prepare('UPDATE RECRUTEUR SET PASSWORD = :nouveau_mdp WHERE EMAIL = :mail');
    ...
    {
        $req = BD::getInstance()->prepare('UPDATE RECRUTEUR SET VALID = 1 WHERE EMAIL = '.$mail);
        $req->execute();
    }
    ...
    {
        $req = BD::getInstance()->prepare('DELETE FROM RECRUTEUR WHERE EMAIL = :pom OR PSEUDONYME = :pom');
 ...
 */
    ...
    {
        $req = BD::getInstance()->prepare("UPDATE RECRUTEUR 
                                           SET VALID = :valide
                                           WHERE EMAIL = `$this->mail`
                                           AND PSEUDONYME = `$this->pseudo`");
